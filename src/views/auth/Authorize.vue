...
async function allowAuth() {
    window.location.href = `${server}/api/v1/oauth/authorization?authorization=${route.query['authorization']}&grant=true`
}

async function denyAuth() {
    window.location.href = `${server}/api/v1/oauth/authorization?authorization=${route.query['authorization']}&grant=false`
}

async function loadAuthorizationInfo() {
    const r = await fetch(`${server}/api/v1/oauth/authorization-info?authorization=${route.query['authorization']}`);
    switch(r.status) {
        case 200:
        interface Response {
            client_name: string,
